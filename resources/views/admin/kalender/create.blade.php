...
<div class="mb-8">
    ...
    <h1 class="text-2xl md:text-3xl font-black text-gray-800 font-['Poppins']">Tambah Kegiatan</h1>
</div>

<form action="{{ route('admin.kalender.store') }}" method="POST" class="bg-white rounded-2xl shadow-md border border-gray-100 p-6 md:p-8 max-w-2xl">
    @csrf

    <div class="mb-5">
        <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Kegiatan</label>
        <input type="text" name="kegiatan" value="{{ old('kegiatan') }}" placeholder="Misal: Libur Semester Ganjil"
    ...
    <div class="mb-6">
        ...
        <textarea name="keterangan" rows="3" class="w-full px-4 py-3 rounded-xl bg-[#EBF5FA] border border-transparent focus:border-[#85C2DB] focus:bg-white outline-none text-sm transition">{{ old('keterangan') }}</textarea>
    </div>
