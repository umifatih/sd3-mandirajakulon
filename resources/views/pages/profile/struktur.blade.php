...
<section class="relative bg-gradient-to-b from-[#8B5E3C]/95 to-[#8B5E3C] pt-24 md:pt-32 pb-16 md:pb-24 text-white text-center overflow-hidden">
...
{{-- Diagram Struktur --}}
<section class="py-16 md:py-24 bg-[#F8F5F2] relative">
    <div class="max-w-7xl mx-auto px-4"
        ...
        <div class="text-center mb-16">
            <span class="text-[#8B5E3C] font-bold tracking-widest uppercase text-xs block mb-3">Komando & Koordinasi</span>
            ...
            <ul class="tree">
                ...
                <li>
                    <div class="node-card" @click="openModal(personil.kepala)">
                        <div class="card-inner bg-white rounded-2xl shadow-lg border border-gray-100 p-5 w-52 md:w-60 text-center cursor-pointer transition hover:shadow-2xl hover:-translate-y-1 hover:border-[#8B5E3C]">
                            <img :src="personil.kepala.foto" class="w-20 h-20 rounded-full object-cover mx-auto border-4 border-[#D9B99B] shadow-md mb-3" alt="foto">
                            <h4 class="font-bold text-gray-800 font-['Poppins'] text-sm md:text-base" x-text="personil.kepala.nama"></h4>
                            <p class="text-xs text-[#8B5E3C] font-semibold mt-1" x-text="personil.kepala.jabatan"></p>
                        </div>
                    </div>
                    <ul>
                        <li>
                            <div class="node-card" @click="openModal(personil.wakil)">
                                <div class="card-inner bg-white rounded-2xl shadow-lg border border-gray-100 p-5 w-52 md:w-60 text-center cursor-pointer transition hover:shadow-2xl hover:-translate-y-1 hover:border-[#8B5E3C]">
                                    <img :src="personil.wakil.foto" class="w-20 h-20 rounded-full object-cover mx-auto border-4 border-[#D9B99B] shadow-md mb-3" alt="foto">
                                    <h4 class="font-bold text-gray-800 font-['Poppins'] text-sm md:text-base" x-text="personil.wakil.nama"></h4>
                                    <p class="text-xs text-[#8B5E3C] font-semibold mt-1" x-text="personil.wakil.jabatan"></p>
                                </div>
                            </div>
                                ...
                                <template x-for="koor in personil.koordinator" :key="koor.nama">
                                    <li>
                                        <div class="node-card" @click="openModal(koor)">
                                            <div class="card-inner bg-white rounded-2xl shadow-lg border border-gray-100 p-4 w-44 md:w-52 text-center cursor-pointer transition hover:shadow-2xl hover:-translate-y-1 hover:border-[#8B5E3C]">
                                                <img :src="koor.foto" class="w-16 h-16 rounded-full object-cover mx-auto border-4 border-[#D9B99B] shadow-md mb-2" alt="foto">
                                                <h4 class="font-bold text-gray-800 font-['Poppins'] text-xs md:text-sm" x-text="koor.nama"></h4>
                                                <p class="text-xs text-[#8B5E3C] font-semibold mt-1" x-text="koor.jabatan"></p>
                                            </div>
                                        </div>
                                    </li>
                                </template>
                        ...
                        <li>
                            <div class="node-card" @click="openModal(personil.tu)">
                                <div class="card-inner bg-white rounded-2xl shadow-lg border border-gray-100 p-5 w-52 md:w-60 text-center cursor-pointer transition hover:shadow-2xl hover:-translate-y-1 hover:border-[#8B5E3C]">
                                    <img :src="personil.tu.foto" class="w-20 h-20 rounded-full object-cover mx-auto border-4 border-[#D9B99B] shadow-md mb-3" alt="foto">
                                    <h4 class="font-bold text-gray-800 font-['Poppins'] text-sm md:text-base" x-text="personil.tu.nama"></h4>
                                    <p class="text-xs text-[#8B5E3C] font-semibold mt-1" x-text="personil.tu.jabatan"></p>
                                </div>
                            </div>
                        </li>
                    </ul>
        ...
        <div x-cloak x-show="showModal" x-transition.opacity.duration.300 class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @click.self="closeModal()">
            <div x-show="showModal" x-transition.scale.90.duration.300 class="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-8 relative border border-[#F8F5F2]">
                ...
                <template x-if="activePerson">
                    <div class="text-center">
                        <img :src="activePerson.foto" class="w-28 h-28 rounded-full object-cover mx-auto border-4 border-[#D9B99B] shadow-xl mb-5" alt="foto">
                        <h3 class="text-2xl font-black text-gray-800 font-['Poppins']" x-text="activePerson.nama"></h3>
                        <p class="text-[#8B5E3C] font-semibold mt-1" x-text="activePerson.jabatan"></p>
                        <p class="text-sm text-gray-500 mt-1" x-text="'NIP. ' + activePerson.nip"></p>
                        <div class="mt-6 p-5 bg-[#F8F5F2] rounded-2xl text-left">
...
<section class="py-20 bg-white relative overflow-hidden">
    <div class="max-w-7xl mx-auto px-6">
        <div class="text-center mb-16">
            <span class="text-[#8B5E3C] font-bold tracking-widest uppercase text-xs block mb-3">Tenaga Pendidik</span>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 lg:gap-10">
            ...
            @endphp
            @foreach($guru as $g)
            <div class="bg-[#F8F5F2] rounded-2xl p-5 text-center border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 group">
                <img src="{{ $g['foto'] }}" class="w-20 h-20 rounded-full object-cover mx-auto border-4 border-white shadow-md group-hover:border-[#D9B99B] transition-colors mb-3" alt="foto">
